...
class MyModule(SimpleBoardTop):
  def __init__(self):
    ...
    self.IndicatorSinkRgbLed = self.Block(IndicatorSinkRgbLed())
    self.CharlieplexedLedMatrix = self.Block(CharlieplexedLedMatrix(nrows=2, ncols=3))

    self.connect(self.Xiao_Esp32c3.gnd)
    self.connect(self.IndicatorSinkRgbLed.signals, self.Xiao_Esp32c3.gpio.request_vector('gpio_13'))
    self.connect(self.IndicatorSinkRgbLed.pwr, self.Xiao_Esp32c3.pwr_out)
...
class CharlieMatrixTestCase(unittest.TestCase):
  def test_compile(self):
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "tests/CharlieMatrixWithRgb.json")) as f:
      netlist_data = f.read()
