...
class MyModule(SimpleBoardTop):
  def __init__(self):
    ...
    self.connect(self.Esp32_Wroom_32.i2c.request('i2c_16'), self.Hdc1080.i2c, self.Bme680.i2c, self.Bh1750.i2c, self.I2cPullup.i2c)
    self.connect(self.Esp32_Wroom_32.pwr, self.I2cPullup.pwr, self.Hdc1080.pwr, self.Bme680.pwr, self.Bh1750.pwr, self.Ldl1117.pwr_out)
"""


class IotSensorThingTestCase(unittest.TestCase):
  def test_compile(self):
    # the server messes with cwd so we need to use the absolute path
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "tests/IotSensorThing.json")) as f:
      netlist_data = f.read()
